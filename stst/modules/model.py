...
class Model(object):
    def __init__(self,
        ...
        self.test_feature_file = config.MODEL_DIR + '/' + self.model_name + '.feature.test.txt'

        self.model_file = config.MODEL_DIR + '/' + self.model_name + '.pkl'

        self.output_file = config.OUTPUT_DIR + '/' + self.model_name + '.output.txt'
    ...
    def train(self, train_instances, train_file, out_list=None):
        ...
        ''' 1. Extract Features '''
        self.make_feature_file(train_instances, train_file, 'train')
    ...
    def test(self, dev_instances, dev_file):
        ...
        ''' 1. Extract Features '''
        if 'test' in dev_file:
            self.make_feature_file(dev_instances, dev_file, 'test')
        else:
            self.make_feature_file(dev_instances, dev_file, 'dev')
        ...
        ''' 2. Predict Answers '''
        if 'test' in dev_file:
            predict_label = self.classifier.test_model(self.test_feature_file, self.model_file, self.output_file)
        else:
    ...
    def make_feature_file(self, train_instances, train_file, type):
        """
        ...
        if type == 'dev':
            ...
            f_feature = utils.create_write_file(self.test_feature_file)
        else:
            # 【】
